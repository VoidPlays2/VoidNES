...
public class Cartridge
{
    ...
    void ParseHeader(BinaryReader reader)
    {
        ...
        {
            Debug.Log("Magic header value (" + magicNum.ToString("X4") + ") is incorrect");
            Invalid = true;
            return;
        }
        ...
        Debug.Log((16 * PrgRomBanks).ToString() + "Kb of PRG ROM");

        // Size of CHR ROM (Or set CHR RAM if using it)
        ChrBanks = reader.ReadByte();
        if (ChrBanks == 0)
        {
            Debug.Log("Cartridge uses CHR RAM");
            ChrBanks = 2;
            UsesChrRam = true;
        }
        else
        {
            Debug.Log((8 * ChrBanks).ToString() + "Kb of CHR ROM");
            UsesChrRam = false;
        }
        ...
        Debug.Log("VRAM mirroring type: " + (VerticalVramMirroring ? "vertical" : "horizontal"));

        BatteryBackedMemory = (_flags6 & 0x02) != 0;
        if (BatteryBackedMemory) Debug.Log("Cartridge contains battery backed persistent memory");

        ContainsTrainer = (_flags6 & 0x04) != 0;
        if (ContainsTrainer) Debug.Log("Cartridge contains a 512 byte trainer");
